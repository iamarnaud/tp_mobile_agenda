...
export class CalendarComponent implements OnInit {
    ...
    deleteEvents(evtID, revision): void {
        // partie entre parenthèses => callback
        this.eventService.deleteEvent(evtID, revision).subscribe(data => {
            location.reload()
        })
    }
    addParticipant(evtID, user, revision, form): void {
        // partie entre parenthèses => callback
        this.eventService
            .addComer(evtID, user, revision, form)
            .subscribe(data => {
                location.reload()
            })
    }
    removeParticipant(evtID, user, revision, form, index): void {
        // partie entre parenthèses => callback
        this.eventService
            .removeComer(evtID, user, revision, form, index)
            .subscribe(data => {
                location.reload()
            })
    }
    ...
    async infoEvent(evt) {
        ...
        const time =
            moment(evt.doc.start_time).format('LT') +
            ' to ' +
            moment(evt.doc.end_time).format('LT')
        ...
        if (this.mine == true) {
            const alert = await this.alertController.create({
                ...
                buttons: [
                    ...
                    },
                    {
                        ...
                        cssClass: 'warning',
                        handler: () => {
                            this.navCtrl.navigateForward(['/update'], true, {
                                queryParams: { evtId: evt.doc._id }
                            })
                        }
                    }
                ]
            ...
            await alert.present()
        } else if (this.comesToEvent === true) {
            const alert = await this.alertController.create({
                header: evt.doc.title,
                subHeader: time,
